...
result_file = '/home/ez/project/result'
now = datetime.now()
...
def object_to_marker(box, frame_id="base", marker_id=None, duration=0.15, color=None):
    ...
    marker.points = line_points_from_3d_bbox(
                        ...
                        box['size'][0], box['size'][2], box['size'][1], Quaternion(orientation).yaw_pitch_roll[0])
...
class GnnPmbNode:
    def __init__(self):
        ...
        config='/home/ez/project/tracking/src/tracking_module/scripts/configs/gnnpmb_parameters.json'
        ...
        self.flag_callback1 = True
        self.flag_callback2 = True
        self.flag_callback3 = True
        self.update_frequency = 8
        self.classifications = ['car']
        # self.classifications = ['pedestrian','car']
        # self.classifications = ['bicycle','motorcycle',  'trailer', 'truck','bus','pedestrian','car']
        ...
        rospy.Subscriber("ego_pose", PoseStamped, self.ego_rec, queue_size=100)
        rospy.Subscriber("lidar_pose", PoseStamped, self.cs_rec, queue_size=100)        
        rospy.Subscriber("estimate", DetectedObjectList, self.bboxes_rec, queue_size=100)



    def cs_rec(self, pose):
        # 等待回调函数1执行完毕
        while not (self.flag_callback1 or self.flag_callback2):
            rospy.sleep(0.1)

        self.cs_pose = pose.pose

        self.flag_callback2 = False
    
    def ego_rec(self, pose):
    ...
    def bboxes_rec(self, boxes):
        ...
        print(f'length of Z_k_all = {self.count}')
        ...
        if self.count  == 0:
            ...
            ego_data['translation'] = [self.ego_pose.position.x, self.ego_pose.position.y, self.ego_pose.position.z]
            ego_data['rotation'] = [self.ego_pose.orientation.w, self.ego_pose.orientation.x, self.ego_pose.orientation.y, self.ego_pose.orientation.z]

            calib_data['translation'] = [self.cs_pose.position.x, self.cs_pose.position.y, self.cs_pose.position.z]
            calib_data['rotation'] = [self.cs_pose.orientation.w, self.cs_pose.orientation.x, self.cs_pose.orientation.y, self.cs_pose.orientation.z]
            ...
            for classification in self.classifications:
                ...
                if self.flag == 0:
                    ...
                    if self.framecount == 20:
                        ...
                        self.framecount += 1
                        self.filter_predicted[classification] = self.gnnpmb_filter[classification].predict(self.ego_pose.position.x,time_lag,self.filter_pruned[classification], Z_k, self.birth_rate[classification])
                ...
                for idx in range(len(estimatedStates_for_this_classification['mean'])):
                    ...
                    temp1 = object_to_marker(instance_info, frame_id='LIDAR_TOP', marker_id=idx, duration= 3.2 / self.update_frequency)
                    temp2 = object_to_marker_labels(instance_info, frame_id='LIDAR_TOP', marker_id=idx, duration= 3.2 / self.update_frequency)
